<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <div class="max-h-96 space-y-1 overflow-y-auto pr-1.5 scrollbar-slim">
        <button
          type="button"
        ...
        <div v-for="category in categories" :key="category.id" class="space-y-0.5">
          ...
          <div v-if="category.children?.length && expandedCategories.has(category.id)" class="ml-4 space-y-0.5 border-l-2 border-slate-200 pl-3">
            <button
...
const props = defineProps<{
  categories: { id: string; name: string; slug: string; count: number; children?: { id: string; name: string; slug: string; count: number }[] }[];
