<template>
  <div class="blog">
    ...
    <div class="blog-menus flex flex-wrap gap-3 items-center border-b-2 mt-10">
      <div
        class="menu-single font-thin cursor-pointer relative group pb-1 hover:text-orange-800"
        v-for="item in blogCategories"
        :key="item.id"
      >
        {{ item.category }}
        <div
          class="absolute left-0 -bottom-0.5 h-0.5 bg-orange-800 w-0 group-hover:w-full transition-all duration-300"
        ></div>
      </div>
    </div>
    <div class="blog-posts flex flex-wrap gap-x-6 gap-y-20 mt-10">
      <div
        class="post-single relative w-full sm:w-[calc(50%-12px)] group"
      ...
      >
        ...
        />
        <div
          class="absolute top-0 right-0 w-0 h-0 border-t-[30px] border-t-transparent group-hover:border-t-black group-hover:border-l-[30px] group-hover:border-l-white transform rotate-180"
        ></div>

        <div
          class="writer-time-category absolute w-full bg-white bg-opacity-30 backdrop-blur-sm text-white left-0 bottom-8 h-24 flex items-center px-5 justify-between"
        >
        ...
        <div class="title mt-2 relative inline-block">
          {{ post.title }}
          <div
